...
create table if not exists public.agency_subscriptions (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (jsonb_typeof(payment_method)='object')
);
...
select a.id,a.plan,case when a.plan='trial' then 'trialing' else 'active' end,coalesce(a.trial_ends_at,now()+interval '1 month')
...
select a.id,jsonb_build_object(
  'welcome',true,
  'company',length(trim(a.name))>0 and a.contact_email is not null,
  'branding',coalesce(a.branding->>'primaryColor','')<>'',
  ...
  'firstDashboard',exists(select 1 from public.dashboards d where d.agency_id=a.id),
  'firstReport',exists(select 1 from public.reports r where r.agency_id=a.id)
),'company'
from public.agencies a
...
alter table public.agency_team_clients enable row level security;
...
drop policy if exists agency_teams_read on public.agency_teams;
...
drop policy if exists agency_team_members_read on public.agency_team_members;
...
drop policy if exists agency_team_clients_read on public.agency_team_clients;
...
drop policy if exists agency_invoices_read on public.agency_invoices;
...
drop policy if exists agency_onboarding_read on public.agency_onboarding_progress;
...
drop trigger if exists agency_teams_touch on public.agency_teams;
...
drop trigger if exists agency_subscriptions_touch on public.agency_subscriptions;
...
drop trigger if exists agency_onboarding_touch on public.agency_onboarding_progress;
...
('agency.read','agency-management','Read agency company and operating settings','standard'),
('agency.manage','agency-management','Update agency company, legal and default settings','critical'),
('users.read','agency-management','Read agency users, teams and assignments','sensitive'),
('users.manage','agency-management','Invite, update and deactivate agency users and teams','critical'),
('billing.read','agency-management','Read subscription, limits, usage and invoices','sensitive'),
('billing.manage','agency-management','Change, cancel or resume agency subscription','critical'),
('onboarding.read','agency-management','Read agency onboarding progress','standard'),
('onboarding.manage','agency-management','Update agency onboarding checklist','sensitive')
