...
create index if not exists workspace_recent_items_user_visited_idx
  on public.workspace_recent_items(user_id, visited_at desc);
create index if not exists workspace_recent_items_agency_visited_idx
  on public.workspace_recent_items(agency_id, visited_at desc);
create index if not exists workspace_recent_items_client_idx
  on public.workspace_recent_items(client_id);
create index if not exists agency_entitlement_overrides_updated_by_idx
  on public.agency_entitlement_overrides(updated_by);
create index if not exists agency_feature_flags_updated_by_idx
  on public.agency_feature_flags(updated_by);
...
drop policy if exists workspace_recent_items_select on public.workspace_recent_items;
create policy workspace_recent_items_select on public.workspace_recent_items
for select to authenticated using (
  user_id = (select auth.uid()) and private.is_agency_member(agency_id)
);

drop policy if exists workspace_recent_items_insert on public.workspace_recent_items;
create policy workspace_recent_items_insert on public.workspace_recent_items
...
drop policy if exists workspace_recent_items_update on public.workspace_recent_items;
create policy workspace_recent_items_update on public.workspace_recent_items
...
drop policy if exists workspace_recent_items_delete on public.workspace_recent_items;
create policy workspace_recent_items_delete on public.workspace_recent_items
...
as $$
  select case when (select auth.uid()) is null then '[]'::jsonb else
    coalesce((
      select jsonb_agg(event order by event_created_at desc)
      from (
        select
          e.created_at as event_created_at,
          jsonb_build_object(
            'id', e.id,
            'action', coalesce(e.payload::jsonb->>'action', e.payload::jsonb->>'log_type', 'auth_event'),
            'createdAt', e.created_at,
            'ipAddress', nullif(e.ip_address, ''),
            'userAgent', coalesce(
              e.payload::jsonb->>'user_agent',
              e.payload::jsonb#>>'{traits,user_agent}'
            ),
            'provider', coalesce(
              e.payload::jsonb->>'provider',
              e.payload::jsonb#>>'{traits,provider}'
            )
          ) as event
        from auth.audit_log_entries e
        where coalesce(
          e.payload::jsonb->>'actor_id',
          e.payload::jsonb->>'user_id',
          e.payload::jsonb#>>'{traits,user_id}'
        ) = (select auth.uid())::text
        order by e.created_at desc
        limit greatest(1, least(coalesce(max_items, 50), 100))
      ) events
    ), '[]'::jsonb)
  end;
$$;

create or replace function public.update_workspace_preferences(
  target_language text default null,
  target_timezone text default null,
  target_theme text default null,
  target_settings jsonb default null
)
returns jsonb
language plpgsql
security definer
...
as $$
declare
  result public.user_preferences%rowtype;
  agency_id uuid;
begin
  if (select auth.uid()) is null then
    raise exception 'AUTH_REQUIRED';
  end if;
  if target_language is not null and target_language not in ('en','ru','kk') then
  ...
  if target_theme is not null and target_theme not in ('system','light','dark') then
    raise exception 'THEME_INVALID';
  end if;
  if target_settings is not null and jsonb_typeof(target_settings) <> 'object' then
  ...
  values(
    (select auth.uid()),
    coalesce(target_language,'ru'),
    coalesce(target_timezone,'Asia/Almaty'),
    coalesce(target_theme,'system'),
    coalesce(target_settings,'{}'::jsonb)
  )
  on conflict(user_id) do update set
    language = coalesce(target_language, public.user_preferences.language),
    timezone = coalesce(target_timezone, public.user_preferences.timezone),
    theme = coalesce(target_theme, public.user_preferences.theme),
    settings = case
      when target_settings is null then public.user_preferences.settings
      else public.user_preferences.settings || target_settings
    end,
    updated_at = now()
  returning * into result;

  agency_id := result.active_agency_id;
  if agency_id is not null and private.is_agency_member(agency_id) then
    perform private.platform_core_write_audit(
      agency_id,
      'workspace.preferences_updated',
      'update',
      'user_preferences',
      (select auth.uid())::text,
      jsonb_build_object(
...
create or replace function public.record_workspace_recent_item(
...
begin
  ...
  if length(trim(coalesce(target_item_type,'')))=0
    or length(trim(coalesce(target_item_id,'')))=0
    or length(trim(coalesce(target_title,'')))=0
    or target_route is null
    or left(target_route,1)<>'/' then
    raise exception 'RECENT_ITEM_INVALID';
  end if;

  insert into public.workspace_recent_items(
    user_id,agency_id,client_id,item_type,item_id,title,route,visited_at
  ) values(
    (select auth.uid()),target_agency_id,target_client_id,
    target_item_type,target_item_id,target_title,target_route,now()
  )
  on conflict(user_id,agency_id,item_type,item_id) do update set
...
active_client as (
  ...
  where not exists(select 1 from prefs)
     or c.id=(select active_client_id from prefs)
  order by c.created_at desc,c.id
  limit 1
),
recent_items as (
  select r.* from public.workspace_recent_items r
  where r.user_id=(select auth.uid())
    and r.agency_id=(select id from active_agency)
...
select jsonb_build_object(
  ...
  'productContext',jsonb_build_object(
    'id','marketing','name','IMDS Marketing','surface','workspace','route','/'
  ),
  ...
  'recentItems',coalesce((select jsonb_agg(jsonb_build_object(
    'id',id,'clientId',client_id,'itemType',item_type,'itemId',item_id,
    'title',title,'route',route,'visitedAt',visited_at
  ) order by visited_at desc) from recent_items),'[]'::jsonb)
);
$$;
