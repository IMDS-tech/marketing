...
export class Db implements OnModuleDestroy{
  private readonly pool=new pg.Pool({connectionString:config.DATABASE_URL,max:10,ssl:config.DATABASE_URL.includes('localhost')?false:{rejectUnauthorized:false}});
