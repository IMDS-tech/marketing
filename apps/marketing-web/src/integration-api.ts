...
export type IntegrationWorkspace={integrations:any[];sources:any[];accounts:any[];jobs:any[];runs:any[]};
export type IntegrationSchema={id:string;slug:string;name:string;category:string;auth_type:string;lifecycle:string;metadata:Record<string,unknown>;entities:any[];fields:any[];errors:any[]};
export const integrationApi={
workspace:(agencyId:string,clientId?:string)=>request<IntegrationWorkspace>(`/v1/integrations/workspace?agencyId=${encodeURIComponent(agencyId)}${clientId?`&clientId=${encodeURIComponent(clientId)}`:''}`),
schema:(agencyId:string,integrationId?:string)=>request<IntegrationSchema[]>(`/v1/integrations/schema?agencyId=${encodeURIComponent(agencyId)}${integrationId?`&integrationId=${encodeURIComponent(integrationId)}`:''}`),
...
updateSource:(dataSourceId:string,body:{syncDepthDays?:number;settings?:Record<string,unknown>})=>request<{id:string;sync_depth_days:number;settings:Record<string,unknown>}>(`/v1/integrations/sources/${dataSourceId}`,{method:'PATCH',body:JSON.stringify(body)}),
retryJob:(jobId:string)=>request<{id:string;state:string}>(`/v1/integrations/jobs/${jobId}/retry`,{method:'POST',body:'{}'}),
