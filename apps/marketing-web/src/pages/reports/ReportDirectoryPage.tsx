...
import{Archive,FilePlus2,Folder,RefreshCw,Search,Send}from'lucide-react';
...
export function ReportDirectoryPage(){const{workspace,session}=useAuth();if(!workspace)return <Card className="loading-card">Загрузка отчётов…</Card>;const agency=workspace.activeAgency;if(!agency||!canAccess(agency,permissions.reportsRead))return <ForbiddenPage/>;return <ReportDirectory workspace={workspace} session={session}/>}

function ReportDirectory({workspace,session}:{workspace:WorkspaceBootstrap;session:Session|null}){
 const{locale}=useI18n();const navigate=useNavigate();const queryClient=useQueryClient();const agency=workspace.activeAgency!;const canManage=canAccess(agency,permissions.reportsManage);
 ...
 const filters={search:search||undefined,status:status||undefined,clientId:clientId||undefined,folderId:folderId||undefined};
 ...
 return <>
  ...
  {showCreate&&<Card className="report-create-card"><form onSubmit={submit}><div><h3>Новый отчёт</h3><p>Создаётся как черновик. Конструктор, расписание и получатели доступны сразу после создания.</p></div><label>Название<input autoFocus value={name} maxLength={160} onChange={event=>setName(event.target.value)} required/></label><label>Клиент<select value={newClientId} onChange={event=>setNewClientId(event.target.value)}><option value="">Агентский отчёт</option>{workspace.clients.map(client=><option key={client.id} value={client.id}>{client.company}</option>)}</select></label><label className="report-description">Описание<textarea value={description} maxLength={2000} onChange={event=>setDescription(event.target.value)}/></label><div className="report-form-actions"><Button type="button" variant="secondary" onClick={()=>setShowCreate(false)}>Отмена</Button><Button type="submit" disabled={createMutation.isPending}>{createMutation.isPending?'Создание…':'Создать'}</Button></div>{createMutation.error&&<p className="report-error">{createMutation.error.message}</p>}</form></Card>}
  <Card className="report-directory-card"><div className="report-toolbar"><div className="report-search"><Search size={17}/><input aria-label="Поиск отчётов" placeholder="Поиск по названию и описанию" value={search} onChange={event=>setSearch(event.target.value)}/></div><select aria-label="Статус" value={status} onChange={event=>setStatus(event.target.value as ReportStatus|'')}><option value="">Все статусы</option>{statuses.map(item=><option key={item} value={item}>{statusLabel[item]}</option>)}</select><select aria-label="Клиент" value={clientId} onChange={event=>setClientId(event.target.value)}><option value="">Все клиенты</option>{workspace.clients.map(client=><option key={client.id} value={client.id}>{client.company}</option>)}</select><select aria-label="Папка" value={folderId} onChange={event=>setFolderId(event.target.value)}><option value="">Все папки</option>{(foldersQuery.data??[]).map(folder=><option key={folder.id} value={folder.id}>{folder.name}</option>)}</select></div>
   {reportsQuery.isLoading?<div className="report-state">Загрузка каталога…</div>:reportsQuery.error?<div className="report-state report-error">Не удалось загрузить отчёты: {reportsQuery.error.message}</div>:reports.length===0?<div className="report-state"><Folder size={32}/><h3>Отчёты не найдены</h3><p>Измените фильтры или создайте первый отчёт.</p></div>:<div className="table-wrap"><table><thead><tr><th>Отчёт</th><th>Клиент</th><th>Папка</th><th>Статус</th><th>Следующий запуск</th><th>Обновлён</th><th/></tr></thead><tbody>{reports.map(report=><tr key={report.id}><td><div className="report-name"><strong>{report.name}</strong><span>{report.description||'Без описания'}</span></div></td><td>{report.clientName??workspace.clients.find(item=>item.id===report.clientId)?.company??'Агентство'}</td><td>{report.folderName??'—'}</td><td><Badge tone={statusTone[report.status]}>{statusLabel[report.status]}</Badge></td><td>{report.nextRunAt?new Date(report.nextRunAt).toLocaleString(locale):'—'}</td><td>{new Date(report.updatedAt).toLocaleDateString(locale)}</td><td><div className="report-row-actions"><button className="report-icon-button" title="Расписание и доставка" onClick={()=>navigate({to:'/reports/$reportId/delivery',params:{reportId:report.id}})}><Send size={17}/></button>{canManage&&report.status!=='archived'&&<button className="report-icon-button" title="Архивировать" onClick={()=>archiveMutation.mutate(report.id)}><Archive size={17}/></button>}</div></td></tr>)}</tbody></table></div>}
  </Card>
 </>;
}
